...
**Async fetch** (`fetch.c`): Non-blocking HTTP GET via libcurl in a detached pthread. `fetch_start()` spawns thread, `fetch_check()` polls status each frame (mutex-protected), `fetch_take_response()` transfers ownership. All three overlays (MUF, Sporadic E, Aurora) plus Kp/Bz indices auto-refresh every 15 minutes (`OVERLAY_UPDATE_SEC`) while active. In-flight fetches are cleaned up at exit.
