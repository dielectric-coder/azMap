...
├── overlay.h/c     MUF contour lines (KC2G GeoJSON) + aurora heatmap (NOAA OVATION) + Kp/Bz indices (NOAA SWPC)
...
**Sidebar panel**: The sidebar is always visible (no toggle). It displays UTC/local clocks, station info from the swl dashboard (received via FIFO or `-d` CLI arg), and distance/azimuth readouts. The bottom of the sidebar has two sections with labeled button groups: "LAYERS" (Aurora, Spor.E, MUF) and "SOURCE" (QRZ, WSJT, BCB). Section labels and horizontal divider lines are rendered as part of the button text geometry in full-window coordinates. When MUF is active, a color-coded legend (swatch line + MHz label per contour level) is rendered above the LAYERS label. When Aurora is active, Kp and Bz geomagnetic indices are displayed right-aligned in the sidebar at the same vertical position as the MUF legend.

**Button system**: Buttons use rounded rectangles (`emit_rounded_rect()` for fill, `emit_rounded_rect_outline()` for border) with per-button vertex offsets/counts. Three visual states: normal (dark), hovered (lighter), active (blue highlight). Active state is indicated by a bitmask (`btn_active_mask`) so multiple buttons can be active simultaneously (e.g. Proj in ortho mode + Aurora + MUF). The Proj button is always in the upper-left of the map; Home is bottom-center of the map; sidebar buttons are positioned in two rows at the bottom of the sidebar. Buttons are drawn in a separate full-window viewport pass (`renderer_draw_buttons()`) after the sidebar, so they can span both map and sidebar areas.
...
**Async fetch** (`fetch.c`): Non-blocking HTTP GET via libcurl in a detached pthread. `fetch_start()` spawns thread, `fetch_check()` polls status each frame (mutex-protected), `fetch_take_response()` transfers ownership. Both overlays auto-refresh every 15 minutes (`OVERLAY_UPDATE_SEC`) while active.
